...
func NewController(kubectlst kubernetes.Interface, clientset clientset.Interface, mksPipelineInformer appsinformers.MksPipelineInformer, redisClient *redis.Client) *controller {
	...
	c := &controller{
		...
		mksPipelineCacheSynced: mksPipelineInformer.Informer().HasSynced, //mksPipelineInformer.Informer().HasSynced,
...
func (c *controller) handleAdd(obj interface{}) {
	klog.Info("\tAdd mkspipeline was called\n")
	db.Increment(rClient, "MKSPIPELINECREATED")
	tp := &tconfig.TektonParam{}
	cs, err := tp.Client()
	if err != nil {
		klog.Fatalf("\tCannot get the tekton client: %v\n", err)
		db.Increment(rClient, "MKSPIPELINEFAILED")
		return
	}
	crobj := obj.(*v1alpha1.MksPipeline)
	p, err := Create(cs, crobj, metav1.CreateOptions{}, crobj.Namespace)
	if err != nil {
		klog.Errorf("\tCannot create mkspipeline: %v\n", err)
		db.Increment(rClient, "MKSPIPELINEFAILED")
		return
	} else {
		klog.Infof("\tMksPipeline %s created: %s\n", p.GetName(), p.GetUID())
		db.Increment(rClient, "MKSPIPELINECOMPLETED")
	}
	c.queue.Add(obj)
}
...
func (c *controller) handleUpdate(old, new interface{}) {
	klog.Info("\tUpdate mkspipeline was called\n")
}

// Delete method of controller for informer function: DeleteFunc
func (c *controller) handleDelete(obj interface{}) {
	klog.Info("\tDelete mkspipeline was called\n")
	tp := &tconfig.TektonParam{}
	cs, err := tp.Client()
	if err != nil {
		klog.Fatalf("\tCannot get the tekton client: %v\n", err)
	...
	delObj := obj.(*v1alpha1.MksPipeline)
	errDel := Delete(cs, metav1.DeleteOptions{}, delObj.Name, delObj.Namespace)
	if errDel != nil {
		klog.Errorf("\tCannot delete mkspipeline: %v", errDel)
		return
	} else {
		klog.Infof("\tMksPipeline %s deleted\n", delObj.GetName())
		db.Increment(rClient, "MKSPIPELINEDELETED")
	}
	c.queue.Add(obj)
}

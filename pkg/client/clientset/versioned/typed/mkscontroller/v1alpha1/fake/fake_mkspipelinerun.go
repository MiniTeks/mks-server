/*
Copyright 2020 The Knative Authors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "github.com/MiniTeks/mks-server/pkg/apis/mkscontroller/v1alpha1"
	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeMksPipelineRuns implements MksPipelineRunInterface
type FakeMksPipelineRuns struct {
	Fake *FakeMkscontrollerV1alpha1
	ns   string
}

var mkspipelinerunsResource = schema.GroupVersionResource{Group: "mkscontroller.example.mks", Version: "v1alpha1", Resource: "mkspipelineruns"}

var mkspipelinerunsKind = schema.GroupVersionKind{Group: "mkscontroller.example.mks", Version: "v1alpha1", Kind: "MksPipelineRun"}

// Get takes name of the mksPipelineRun, and returns the corresponding mksPipelineRun object, and an error if there is any.
func (c *FakeMksPipelineRuns) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.MksPipelineRun, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(mkspipelinerunsResource, c.ns, name), &v1alpha1.MksPipelineRun{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.MksPipelineRun), err
}

// List takes label and field selectors, and returns the list of MksPipelineRuns that match those selectors.
func (c *FakeMksPipelineRuns) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.MksPipelineRunList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(mkspipelinerunsResource, mkspipelinerunsKind, c.ns, opts), &v1alpha1.MksPipelineRunList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.MksPipelineRunList{ListMeta: obj.(*v1alpha1.MksPipelineRunList).ListMeta}
	for _, item := range obj.(*v1alpha1.MksPipelineRunList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested mksPipelineRuns.
func (c *FakeMksPipelineRuns) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(mkspipelinerunsResource, c.ns, opts))

}

// Create takes the representation of a mksPipelineRun and creates it.  Returns the server's representation of the mksPipelineRun, and an error, if there is any.
func (c *FakeMksPipelineRuns) Create(ctx context.Context, mksPipelineRun *v1alpha1.MksPipelineRun, opts v1.CreateOptions) (result *v1alpha1.MksPipelineRun, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(mkspipelinerunsResource, c.ns, mksPipelineRun), &v1alpha1.MksPipelineRun{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.MksPipelineRun), err
}

// Update takes the representation of a mksPipelineRun and updates it. Returns the server's representation of the mksPipelineRun, and an error, if there is any.
func (c *FakeMksPipelineRuns) Update(ctx context.Context, mksPipelineRun *v1alpha1.MksPipelineRun, opts v1.UpdateOptions) (result *v1alpha1.MksPipelineRun, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(mkspipelinerunsResource, c.ns, mksPipelineRun), &v1alpha1.MksPipelineRun{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.MksPipelineRun), err
}

// Delete takes name of the mksPipelineRun and deletes it. Returns an error if one occurs.
func (c *FakeMksPipelineRuns) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteActionWithOptions(mkspipelinerunsResource, c.ns, name, opts), &v1alpha1.MksPipelineRun{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeMksPipelineRuns) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(mkspipelinerunsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.MksPipelineRunList{})
	return err
}

// Patch applies the patch and returns the patched mksPipelineRun.
func (c *FakeMksPipelineRuns) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.MksPipelineRun, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(mkspipelinerunsResource, c.ns, name, pt, data, subresources...), &v1alpha1.MksPipelineRun{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.MksPipelineRun), err
}
